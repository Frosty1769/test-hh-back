...
class ItemController {
  selectItem(id, isSelected, res) {
    ...
    if (index === -1) {
      res.status(500).json({
        status: "error",
        message: `get id: ${id} with ${isSelected}`
      });
      return;
    }
  ...
  getAllList(page, res, search) {
    ...
    res.status(200).json({
      "status": 'success',
      "data": dataList.filter(it => search ? it.text.includes(search) : true).filter((it, ind) => ind >= offset && ind < 20 * page)
    });
  }
}
